...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // inputs, inputScores 는 SelectorAll() 을 사용하여
   ...
   * 6. 다시 (새로고침) 시작했을 때 localStorage 에 보관된 데이터를
   *        읽어와서 table 에 보이기
   */
  ...
  const scoreCheck = () => {
    ...
    for (let i = 0; i < inputScores.length; i++) {
       ...
       *        검사 과정을 즉시 중단한다
       *        그리고 return false 로 false 값을 되돌려준다
       *
       */
      if (!input.value) {
        // 1
      ...
      // input.select();
      // 입력된 text 에 선택 block 을 설정하여
      // 즉시 문자열을 입력하면  text를 지우고 다시 입력
      // 방향키를 누르면 문자열 중간의 글자들을 수정할 수 있도록
      // 하여주는 역할
      input.select();
      input.focus();
      return false;
  ...
  const scoreInput = () => {
    ...
    /**
     * 한 사람의 성적에는 5개의 항목(학번, 이름, 국어, 영어, 수학)이 있다
    ...
    // cf) 반복문으로 총점을 처리하면 나중에 과목이 추가되어도 문제 없이 동작
    ...
    // 여기에 다다르면 score 는 소멸되고 (Garbage Collecting)
    // scores 는 score 들을 담고 계속 유지가 되고 있다(score 배열은 최상단에 선언됨)
  };
  ...
  const scorePrint = () => {
    ...
    for (let i = 0; i < scores.length; i++) {
      ...
      td = document.createElement("TD");
      td.textContent = score.total;
      tr.appendChild(td);

      td = document.createElement("TD");
      td.textContent = score.avg;
      tr.appendChild(td);
  ...
  const scoreLoad = () => {
    ...
    // loadScore !== ""
    if (loadScore) {
      // scores 를 const 로 선언했을 경우 배열 전체를 재할당하는 것이므로 오류 발생
      scores = JSON.parse(loadScore);
    }
    console.log(scores);
  };

  // script가 html의 위에 있을 경우 ?. 를 필수로 사용할 것
...
// 관점의 분리 : 모듈의 응집도를 높이고 결합도를 낮추는 것
