...
// 선언되지 않은 변수를 사용하려고 할 때 경고 또는 오류
"use strict";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const tbodyStInfo = document.querySelector("tbody");
  let studentList = [];

  // 추가버튼을 클릭했을 때 배열에 데이터 추가
  /**
   * input tag 에 입력한 데이터를 객체로 만들기
   * 객체를 배열에 추가하기
   */
  const addStudent = () => {
    // 학생정보를 객체로 만들기
  ...
  const printStudent = () => {
    ...
    studentList.forEach((학생) => {
      ...
      // 학생 객체 변수에 복사하기
      // 학생 객체는 학생 정보 요소(속성)들의 데이터를
      // 담고있는 배열이 된다

      // forEach 에서 tr, td 변수는 계속 GC 되므로 const 가 가능
      // 이전의 let 사용 이유는 반복하지 않고 계속 재할당했기 때문
